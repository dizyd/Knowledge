...
bayes <- list(chains     = 4,
              ...
              prior_OME = c(
                ...
                prior(normal(0, 1),      class = "b", dpar ="", coef = "est_criterionKcal"),
                prior(student_t(3,0, 1), class = "b", dpar ="", coef = "match_domain:est_criterionKcal"),
                prior(inv_gamma(5,1),    class = "sd"),
                prior(inv_gamma(5,1),    class = "sigma"),
                prior(lkj(2),            class = "cor")),
              prior_rank = c(
                prior(normal(0, 1),   class = "Intercept"),
                prior(normal(0, 1),   class = "b", dpar ="", coef = "match_domain"),
                prior(normal(0, 1),   class = "b", dpar ="", coef = "est_criterionKcal"),
                prior(normal(0, 1),   class = "b", dpar ="", coef = "match_domain:est_criterionKcal"),
                prior(inv_gamma(5,1), class = "sd"),
                prior(inv_gamma(5,1), class = "sigma"))
)
...
res_OME_m2 <-  brm(OME_corr     ~ match_domain * est_criterion + (1|ID) + (est_criterion*match_domain|ID_item),
                   ...
                   cores        = bayes$cores,
                   file         = "Results/Models/fit_H3a_M2_sensitivity.rds",
                   backend      = "rstan",
                   sample_prior = "yes",
...
res_OME_m1 <-  brm(OME_corr    ~ match_domain + est_criterion + (1|ID) + (est_criterion*match_domain|ID_item),
                  ...
                  seed         = bayes$bayes_seed,
                  cores        = bayes$cores,
                  file         = "Results/Models/fit_H3a_M1_sensitivity.rds",
                  backend      = "rstan",
                  sample_prior = "yes",
                  save_pars    = save_pars(all=TRUE))
...
res_OME_m0 <-  brm(OME_corr     ~ match_domain + (1|ID) + (est_criterion*match_domain|ID_item),
                   ...
                   seed         = bayes$bayes_seed,
                   cores        = bayes$cores,
                   file         = "Results/Models/fit_H3a_M0_sensitivity.rds",
                   backend      = "rstan",
                   sample_prior = "yes",
                   save_pars    = save_pars(all=TRUE))
...
bayes_factor(res_OME_m1, res_OME_m0) # 136239773130080096.00000
bayes_factor(res_OME_m2, res_OME_m1) # 24.60190
...
res_rank_m2  <- brm(rank_z       ~ match_domain*est_criterion + (1|ID),
                    ...
                    seed         = bayes$bayes_seed,
                    cores        = bayes$cores,
                    file         = "Results/Models/fit_H3b_M2_sensitivity.rds",
                    backend      = "rstan",
                    sample_prior = "yes",
                    save_pars    = save_pars(all=TRUE))
...
res_rank_m1  <- brm(rank_z       ~ match_domain+est_criterion + (1|ID),
                    ...
                    seed         = bayes$bayes_seed,
                    cores        = bayes$cores,
                    file         = "Results/Models/fit_H3b_M1_sensitivity.rds",
                    backend      = "rstan",
                    sample_prior = "yes",
                    save_pars    = save_pars(all=TRUE))
...
res_rank_m0  <- brm(rank_z       ~ match_domain + (1|ID),
                    ...
                    seed         = bayes$bayes_seed,
                    cores        = bayes$cores,
                    file         = "Results/Models/fit_H3b_M0_sensitivity.rds",
                    backend      = "rstan",
                    sample_prior = "yes",
                    save_pars    = save_pars(all=TRUE))
...
bayes_factor(res_rank_m1, res_rank_m0) # 86.70636
bayes_factor(res_rank_m2, res_rank_m1) # 0.18520
